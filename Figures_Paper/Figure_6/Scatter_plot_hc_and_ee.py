...
fraction = 80.0/100 #Desired percent of extreme events. This is performed interatively in a HPC with more data to obtain Figure 5.  HC = 6^, EE = 80%
...
plt.scatter(labels_test,predict_all,color='lightgray',s=4)
plt.scatter(labels_test_high,predicted_high_test,s=4)
plt.plot([0,max(labels_test_high)*1.02],[0,max(labels_test_high)*1.02],'orange',linewidth=2)
plt.title('Extreme events sampling',fontsize=14)

plt.xlabel('$\Delta tp$ as predicted by traffic model',fontsize=14)
plt.ylabel('$\Delta tp$ as predicted by neural network',fontsize=14)
plt.legend(['Identity line','Not extreme events','Extreme events'],fontsize = 14)
plt.show()
