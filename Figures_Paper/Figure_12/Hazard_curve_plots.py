...
def hazard_curve(rate_dict,metric_dict):
    list_values = metric_dict.values()
    ...
    for value in list_values:
        i = 0
        rate_value = 0
        for value_compar in list_values:
            if value_compar >= value:
                rate_value += rate_list[i]
            i+=1
        lambda_list.append(rate_value)
    lists_plot= sorted(itertools.izip(*[list_values, lambda_list]))
    lambda_list_plt, list_values_plt= list(itertools.izip(*lists_plot))
    return lambda_list_plt,list_values_plt
...
def performance_function_hazard(retrofitted,sa_matrix,scenario_indexes,rates,runs,master_dict,mlp):
    ...
    for idx_scenario in scenario_indexes:
        ...
        dict_lambda[j] = rate_scenario
        j += 1
        damaged_bridges_set,a,b = damage_bridges(lnsas,master_dict,retrofitted,runs)
        performance_scenario = Compute_Performance_Network(damaged_bridges_set,master_dict,mlp)/time_base*100
        dict_performance[j] = performance_scenario
    return dict_lambda, dict_performance
...
rank_OAT = pickle.load(open('OATrank.pkl', 'rb'))
rank_sensitivity = pickle.load(open('capacity.pkl', 'rb'))
rank_vuln = pickle.load(open('vulnerability.pkl', 'rb'))
with open('model_fitted_resampled12732.pkl', 'rb') as f:
    mlp = pickle.load(f)

retro_num = 300 #number of bridges to be retrofitted
ranking_analyzed = rank_OAT #Selection of ranking to evaluate performance
top_bridges = top_n(ranking_analyzed,retro_num)
...
dict_lambda_500, dict_performance_500 = performance_function_hazard(retrofitted,sa_matrix,scenario_indexes,rates,runs,master_dict,mlp)


with open('traffic_model_500.pkl','rb') as f:
    dict_performance_traffic_500 = pickle.load(f)
#This is the value of network performance for the given ranking and number of selected bridges


retro_num = 1 #number of bridges to be retrofitted
...
dict_lambda_1, dict_performance_1 = performance_function_hazard(retrofitted,sa_matrix,scenario_indexes,rates,runs,master_dict,mlp)

with open('traffic_model_1.pkl','rb') as f:
    dict_performance_traffic_1 = pickle.load(f)

vec_x_1_tm, vec_y_1_tm = hazard_curve(dict_lambda_1,dict_performance_traffic_1)
vec_x_1_nn, vec_y_1_nn = hazard_curve(dict_lambda_1,dict_performance_1)

vec_x_500_tm, vec_y_500_tm = hazard_curve(dict_lambda_500,dict_performance_traffic_500)
vec_x_500_nn, vec_y_500_nn = hazard_curve(dict_lambda_500,dict_performance_500)

plt.semilogy(vec_x_1_tm,vec_y_1_tm,color = 'orange')
plt.semilogy(vec_x_1_nn,vec_y_1_nn,color = 'orange',linestyle = 'dashed')

plt.semilogy(vec_x_500_tm,vec_y_500_tm,color = 'royalblue')
plt.semilogy(vec_x_500_nn,vec_y_500_nn,color = 'royalblue',linestyle = 'dashed')
plt.legend(['Traffic model - No retrofitting','Neural network - No retrofitting','Traffic model - 300 bridges','Neural network - 300 bridges'])
plt.xlabel('Change in traffic performance metric $\Delta tp$',fontsize = 14)
plt.ylabel('Annual rate of exceedance',fontsize = 14)
plt.savefig('Hazard_github.eps')
plt.show()